...
- **B4** (fixed): portable Caddy logged `permission denied` writing to `~/Library/Application Support/Caddy/` (autosave.json / instance.uuid / locks/) — that path was root-owned from a previous installed-mode daemon run. Fix: `run.sh` now exports `XDG_DATA_HOME=$GENERATED_DIR` and `XDG_CONFIG_HOME=$GENERATED_DIR` for the Caddy child, so Caddy writes to `./generated/caddy/` instead. Self-contained, gets cleaned by `./uninstall.sh`. Verified: post-fix log shows `autosaved config … "file": "…/generated/caddy/autosave.json"` instead of the three permission-denied errors.
